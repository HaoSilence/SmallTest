package com.xiaofuquan.android.toc.lib.style.autoscrollviewpager;
...
public class AutoScrollViewPager extends ViewPager {
    ...
    public void setAdapter(PagerAdapter adapter) {
        ...
        wrapperPagerAdapter = (wrappedPagerAdapter == null) ? null : new AutoScrollPagerAdapter(
                adapter);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (MotionEventCompat.getActionMasked(ev)) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ((int) Math.abs(mLastMotionX - mInitialMotionX) > touchSlop || (int) Math.abs(
                        mLastMotionY - mInitialMotionY) > touchSlop) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if ((int) mInitialMotionX != 0 && (int) mInitialMotionY != 0) {
                    if ((int) Math.abs(
                            mLastMotionX - mInitialMotionX) < touchSlop && (int) Math.abs(
                            mLastMotionY - mInitialMotionY) < touchSlop) {
                        mInitialMotionX = 0.0f;
                        mInitialMotionY = 0.0f;
                        mLastMotionX = 0.0f;
    ...
    private class InnerOnPageChangeListener implements OnPageChangeListener {
        ...
        public void onPageSelected(final int position) {
            //            if (listener != null && position != 0 && position < wrappedPagerAdapter
            // .getCount() + 1) {
            if (listener != null) {
                final int pos;
                // Fix position
                ...
                // Comment this, onPageSelected will be triggered twice for position 0 and
                // getCount -1.
